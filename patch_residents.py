...
residents_init = """
    ...
    for (let h = 0; h < 6; h++) {
      ...
      // Add a couple of residents per lot
...
content = re.sub(
    r"let harmonyFlowerGrowth = 0;.*?for \(let j = 0; j < 25; j\+\+\) {\s*flowerBeds\.push\([^)]+\);\s*}\s*}\s*}",
    residents_init,
    content,
    flags=re.DOTALL
)

# Replace the harmony condition:
content = re.sub(
    r"const isGoodPlanning = isAfterQ8 && gaugePercent > 10 && gaugePercent <= 90 && !hasBurningCars && !isRioting && activeVehicles\.length > 0;",
    r"const isGoodPlanning = isAfterQ8 && gaugePercent < 50 && !hasBurningCars && !isRioting && activeVehicles.length > 0;",
    content
)
