...
# I will manually reconstruct the step 2 block to make it perfectly match the requested order and a clean single-column/responsive flow.
...
step2_replacement = """  // Step 2: Policy Details & Feedback
  return (
    <div className="w-full max-w-4xl mx-auto h-full flex flex-col gap-3 p-2 sm:p-3 md:p-4 overflow-y-auto">
      {/* Feedback Section at the very top */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-stretch">
        ...
        {/* Left Column: Results & Beliefs */}
        <div className="flex flex-col gap-3">
          {/* Compass Result */}
          <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-xs flex-shrink-0">
            <h3 className="text-[0.625rem] sm:text-xs font-bold uppercase tracking-wider text-gray-500 mb-2 flex items-center gap-1.5">
              <Compass className="w-4 h-4 text-[#0081BC]" />
              Your Curbside Compass Result
            </h3>
            <div className="bg-blue-50/80 border border-blue-100 rounded-lg px-3 py-2 mb-2.5 flex items-center text-xs sm:text-sm gap-2">
              <div className="w-6 h-6 rounded-md flex items-center justify-center text-white shadow-xs flex-shrink-0" style={{ backgroundColor: persona.badgeColor }}>
                <Award className="w-3.5 h-3.5" />
              </div>
              <span className="font-bold text-[#005087] truncate">
                {persona.title}
              </span>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-gray-50 p-2.5 rounded-lg border border-gray-200">
                <span className="text-gray-500 block text-[0.625rem] mb-0.5">Curbside Fee Model</span>
                <span className="font-bold capitalize text-gray-800 truncate block text-[0.6875rem]">
                  {config.curbsideFeeModel}
                </span>
              </div>
              <div className="bg-gray-50 p-2.5 rounded-lg border border-gray-200">
                <span className="text-gray-500 block text-[0.625rem] mb-0.5">Enforcement Level</span>
                <span className="font-bold capitalize text-gray-800 flex items-center gap-1.5 truncate text-[0.6875rem]">
                  <Shield className="w-3.5 h-3.5 text-[#009A44] flex-shrink-0" />
          ...
          {/* You Believe */}
          <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-xs flex flex-col items-center justify-center flex-grow">
            <h4 className="text-[0.625rem] sm:text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">
              You Believe
            </h4>
            <ul className="space-y-1.5 text-xs sm:text-sm text-gray-800 w-full px-2">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-xs flex flex-col">
          {/* City of Edmonton Alignment - Postscript */}
